...
radar_path = data_root + 'single_sensor_data/bag1/radar'
radar_file = os.listdir(radar_path)
radar_file.sort()
radar_timestamp = [int(x[:-4]) for x in radar_file]

all_cart_out = data_root + 'single_sensor_data/bag1/radar_img'
if not os.path.exists(all_cart_out):
  os.mkdir(all_cart_out)

cart_0_n200 = data_root + 'single_sensor_data/bag1/radar_cart_0_n200'
if not os.path.exists(cart_0_n200):
  os.mkdir(cart_0_n200)

#==============================================================================================================================================

def radar_polar_to_cartesian(azimuths, fft_data, radar_resolution=0.175, cart_resolution=0.175, cart_pixel_width=5712, interpolate_crossover=True):
...
img_h, img_w = 5710, 5710 #5712, 5712
devision = 5
sub_crop = np.array_split(np.arange(img_h), devision)

for idx, rt in enumerate(tqdm(radar_timestamp[:])):

	id_list = [rt, radar_timestamp[radar_timestamp.index(rt)]]  #[rt, radar_timestamp[radar_timestamp.index(rt) + 1]]
	img_list = []
	...
	cv2.imwrite(os.path.join(all_cart_out, "{}.png".format(rt)), combined_img)
	...
	cv2.imwrite(os.path.join(cart_0_n200, "{}.png".format(rt)), out_img)
